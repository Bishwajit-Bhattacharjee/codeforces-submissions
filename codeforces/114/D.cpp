...
int main()
{
    ...
    Rolling_hash[0][0] = t[0] - 'a'+ 1;
    Rolling_hash[1][0] = t[0] - 'a' + 1;

    for(int i = 1; i < t.size(); i++)
    {
        for(int mi = 0; mi < 2; mi++)
        {
            Rolling_hash[mi][i] = (Rolling_hash[mi][i-1]*B[mi] + t[i] -'a' + 1) % M[mi] ;
    ...
    for(int i =0 ; i < t.size(); i++)
    {
        for(int j = i; j < t.size() ; j++)
        {
             ...
             if(substrHash(i,i+st.size()-1) == H_st && substrHash(j - en.size() + 1,j) == H_en)
